...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extract balance sheet level program related investments data from IRS 990 filings'
    )
    parser.add_argument(
        '--force-process',
        action='store_true',
    ...
    output_dir = Path("./data/extracted_irs_data/program_related_investments_summary")
    output_dir.mkdir(parents=True, exist_ok=True)
    output_prefix = 'program_related_investments_summary'
    ...
    DETAILS_SUBROOTS = {
        '990': '',
        '990PF': 'PFQualifyingDistributionsGrp',
        '990EZ': ''
    }

    DETAILS_FIELDS = {
        'expenses_and_contributions_amt': {
                '990': '',
                '990PF': 'ExpensesAndContributionsAmt',
                '990EZ': ''
            },
            # Total grants
            'program_related_investments_amt': {
                '990': '',
                '990PF': 'ProgramRelatedInvstTotalAmt',
                '990EZ': ''
            },
            # Total assets
            'total_qualifying_distributions_amt': {
                '990': '',
                '990PF': 'QualifyingDistributionsAmt',
                '990EZ': ''
            }
    }
