"""
Compiler for direct charitable activity details.
Transforms wide format (Description1/Expenses1, Description2/Expenses2, etc.) 
into long format with one row per activity.
...
def reshape_to_long_format(df: pd.DataFrame, id_vars: list) -> pd.DataFrame:
    """
    Reshape from wide format (Description1/Expenses1, etc.) to long format.
    ...
    df_desc = pd.melt(
        df,
        id_vars=id_vars,
        value_vars=['Description1Txt', 'Description2Txt',
                    'Description3Txt', 'Description4Txt'],
        var_name='desc_num',
    ...
    df_exp = pd.melt(
        df,
        id_vars=id_vars,
        value_vars=['Expenses1Amt', 'Expenses2Amt',
                    'Expenses3Amt', 'Expenses4Amt'],
        var_name='exp_num',
...
def main():
    ...
    # Load direct charitable activity details
    print("Loading direct charitable activity details...")
    dca_data_dir = Path("data/extracted_irs_data/direct_charitable_activity_details")
    df_dca = load_csv_files(dca_data_dir, dca_data_dtypes)

    # Reshape from wide to long format
    print("Reshaping to long format...")
    df_dca_long = reshape_to_long_format(df_dca, id_vars)
    ...
    df_dca_final = df_dca_long.merge(
        distinct_filings, how='inner', on=id_vars
    )

    # Save output
    print("Saving compiled direct charitable activity details...")
    output_dir = Path("data/compiled_irs_data")
    output_dir.mkdir(parents=True, exist_ok=True)

    df_dca_final.to_parquet(
        output_dir / 'direct_charitable_activity_details_compiled.parquet',
        engine='fastparquet'
    )

    print(f"Compiled {len(df_dca_final):,} charitable activity records")
